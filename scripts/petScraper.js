    ...
    for (let pageNum = 1; pageNum <= 31; pageNum++) {
      ...
      petItems.forEach((item, index) => {
        try {
            ...
            ddElements.forEach(dd => {
              ...
              if (divs.length >= 2) {
                ...
                spans.forEach(span => {
                  ...
                  if (classList.contains('color_bg') && !classList.contains('color_bg2') && !classList.contains('color_bg3') && !classList.contains('color_bg4')) {
                    elementStats.earth++;
                  }
                });
              }
          ...
          // 초기치 정보 찾기 - 여러 패턴 시도
          let baseStatsMatch = itemText.match(/초기치[:\s]*(\d+)[:\s]*(\d+)[:\s]*(\d+)[:\s]*(\d+)/);
          
          if (!baseStatsMatch) {
            // 다른 패턴으로 시도
            baseStatsMatch = itemText.match(/공격력[:\s]*(\d+)[^\d]*방어력[:\s]*(\d+)[^\d]*순발력[:\s]*(\d+)[^\d]*내구력[:\s]*(\d+)/);
          }
          
          if (!baseStatsMatch) {
            // 또 다른 패턴으로 시도 (숫자만 연속으로)
            const statsNumbers = itemText.match(/(?:초기치|공격력)[^0-9]*(\d+)[^0-9]*(\d+)[^0-9]*(\d+)[^0-9]*(\d+)/);
            if (statsNumbers) {
          ...
          // 성장률 정보 찾기 - 정확한 패턴 사용
          const growthStatsMatch = itemText.match(/성장률[^0-9]*([0-9.]+)[^0-9]*([0-9.]+)[^0-9]*([0-9.]+)[^0-9]*([0-9.]+)/);
          ...
          // 판매등급 찾기
          const gradeMatch = itemText.match(/(일반등급|일반페트|일반|고급등급|고급페트|고급|희귀등급|희귀페트|희귀|영웅등급|영웅페트|영웅|전설등급|전설페트|전설)/);
          if (gradeMatch) {
            grade = gradeMatch[1];
          }
