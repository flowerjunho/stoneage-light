  ...
  const calculatedData = useMemo(() => {
    ...
    while (!converged && iterations < maxIterations) {
      ...
      for (let rebirthIdx = 0; rebirthIdx < 5; rebirthIdx++) {
        ...
        calculatedAgi[rebirthIdx] = Math.max(
          0,
        ...
        const totalDisplay =
          displayStats.con + displayStats.wis + displayStats.dex + displayStats.agi;
        ...
        if (rebirthIdx === 0) {
          ...
          if (prevData) {
            ...
            realStats = {
              con:
                displayStats.con +
                (prevData.appliedRebirthDecimal.con - prevData.appliedRebirth.con),
              wis:
                displayStats.wis +
                (prevData.appliedRebirthDecimal.wis - prevData.appliedRebirth.wis),
              dex:
                displayStats.dex +
                (prevData.appliedRebirthDecimal.dex - prevData.appliedRebirth.dex),
              agi:
                displayStats.agi +
                (prevData.appliedRebirthDecimal.agi - prevData.appliedRebirth.agi),
            };
          } else {
            realStats = { ...displayStats };
          }
        ...
        const rebirthCoeff = Math.trunc(
          totalReal / 12 + cumulativeQuests / 4 + (cumulativeLevels - 85 * (rebirthIdx + 1)) / 4
        );
        ...
        const appliedTotal =
          appliedRebirth.con + appliedRebirth.wis + appliedRebirth.dex + appliedRebirth.agi;
        ...
        if (rebirthIdx === 0) {
          ...
          remainingPoints =
            newData[rebirthIdx - 1].finalRebirthValue + 3 * (level - 1) - totalDisplay;
        }
